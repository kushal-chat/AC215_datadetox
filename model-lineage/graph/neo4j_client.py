...
class Neo4jClient:
    ...
    def create_relationship(self, relationship: Relationship):
        ...
        source_id_field = "model_id" if relationship.source_type == "model" else "dataset_id"
        target_id_field = "model_id" if relationship.target_type == "model" else "dataset_id"
    ...
    def load_graph(self, graph_data: GraphData):
        ...
        logger.info(f"Loading {len(graph_data.models)} models, "
        ...
        for relationship in graph_data.relationships:
            try:
                ...
                logger.warning(f"Failed to create relationship {relationship.source} -> {relationship.target}: {e}")
